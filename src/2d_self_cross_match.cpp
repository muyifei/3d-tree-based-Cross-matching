...
inline double dist2(unsigned int a, unsigned int b){ // 
  return (s[a].x - s[b].x) * (s[a].x - s[b].x) * cos((s[a].y + s[b].y)/360*M_PI) +
         (s[a].y - s[b].y) * (s[a].y - s[b].y);
}
...
double dist4(unsigned int a, unsigned int b){
	...
	x2 = cos(s[b].x)*sin((M_PI_2-s[b].y));
	y2 = sin(s[b].x)*sin((M_PI_2-s[b].y));
	z2 = cos((M_PI_2-s[b].y));
...
int main(int argc, char* argv[]){
    ...
    char filePath[40] = "../data/short_sdssdr12"; // wise
    //char filePath[40] = "../data/short_psc_all";
    //char filePath[40] = "../data/short_sdssdr12";

    unsigned int cnt = 0, cntLimit = 1231051050; //747634026
    //u 1231051050,  470992970, 747634026
    if (argc > 1) cntLimit = atoi(argv[1]);
    if (argc > 2) queryThreadNum = atoi(argv[2]);
	if (argc > 3) curThreadNum = atoi(argv[3]);
	if (argc > 4){
		if (strcmp(argv[4], "wise") == 0){
			strcpy(filePath, "../data/short_wise");
			cntLimit = 747634026;
		}else if(strcmp(argv[4], "sdss") == 0){
	...
	for (long i=0; i<queryThreadNum; i++){
		threadArg[i].l = 1;
		threadArg[i].r = cnt;
		threadArg[i].x = (unsigned int)(i*cnt/queryThreadNum+1);
		threadArg[i].xMax = (unsigned int)((i+1)*cnt/queryThreadNum);
	}
